...
Buy 1 option, Sell 2+ at different strike (typically 1:2 ratio)
Can be call or put ratio spread - generates credit but has unlimited risk on one side
"""

from .base_strategy import BaseStrategy

class RatioSpread(BaseStrategy):
    """Ratio Spread - Asymmetric risk/reward play"""

    strategy_direction = 'neutral'

    @staticmethod
    def generate_code(config):
        """Generate complete Ratio Spread algorithm"""
        ...
        # Ratio spread parameters (call ratio by default)
        formatted_deltas = {
            'long_delta': 0.50,   # ATM long call
            'short_delta': 0.30,  # OTM short calls
            'ratio': 2            # Sell 2 for every 1 bought
        }
        ...
        config_json = RatioSpread._build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config)
        ...
        indicator_manager = BaseStrategy.generate_indicator_manager(indicators_config, RatioSpread.strategy_direction)
        advanced_filters = BaseStrategy.generate_advanced_filters(advanced_config)
        strike_selector = RatioSpread._generate_strike_selector(advanced_config)
        strategy_class = RatioSpread._generate_strategy_class()
        main_algorithm = RatioSpread._generate_main_algorithm(flat_config, advanced_config)
...
"""
    ...
    def _build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config):
        ...
        config_str += f"        'strategy': 'Ratio Spread',\n"
        ...
        config_str += f"            'ratio': {formatted_deltas.get('ratio', 2)}\n"
        config_str += "        },\n"
...
class StrikeSelector:
    """Find strikes for Ratio Spread"""

    @staticmethod
    def find_strikes_by_delta(option_chain, underlying_price, target_deltas, config, algo):
        """Find ratio spread strikes (1 long, 2 short at different strike)"""
...
'''
        ...
        code += '''
            # Find long call (ATM ~0.50 delta)
            long_call = StrikeSelector._find_closest_delta(
                calls, target_deltas.get('long_delta', 0.50), algo
            )

            # Find short call (OTM ~0.30 delta)
            short_call = StrikeSelector._find_closest_delta(
                calls, target_deltas.get('short_delta', 0.30), algo
            )

            if long_call and short_call:
                # Short call should be at higher strike (OTM)
                if short_call.Strike > long_call.Strike:
                    algo.Log(f"[+] Valid Ratio Spread! Long: {long_call.Strike}, Short: {short_call.Strike} (x{target_deltas.get('ratio', 2)})")
                    return {
                        'long_call': long_call,
...
class RatioSpreadStrategy:
    ...
    def enter_position(self, strikes, underlying_price, time):
        ...
        try:
            ...
            ratio = self.algo.config.deltas.get('ratio', 2)

            # Buy 1, Sell ratio (typically 2)
            net_credit_or_debit = (ratio * short_call_price - long_call_price) * 100

            if net_credit_or_debit > 0:
                trade_type = "CREDIT"
            else:
                trade_type = "DEBIT"
                net_credit_or_debit = abs(net_credit_or_debit)

            # Max profit at short strike
            max_profit = ((strikes['short_call'].Strike - strikes['long_call'].Strike) * 100) + (net_credit_or_debit if trade_type == "CREDIT" else -net_credit_or_debit)
            # Max loss is unlimited above short strike (naked portion)
            max_loss = 999999

            self.algo.Log("")
            self.algo.Log("=" * 60)
            self.algo.Log(f">>> ENTERING RATIO SPREAD ({trade_type})")
            self.algo.Log("=" * 60)
            self.algo.Log(f"Underlying: ${underlying_price:.2f}")
            self.algo.Log(f"Long Call: {strikes['long_call'].Strike} (x1)")
            self.algo.Log(f"Short Call: {strikes['short_call'].Strike} (x{ratio})")
            self.algo.Log(f"Net {trade_type}: ${net_credit_or_debit:.2f}")
        ...
        try:
            ...
            for chain in data.OptionChains:
                ...
                if len(current_prices) == 2:
                    current_value = (current_prices['lc'] - ratio * current_prices['sc']) * 100
                    entry_value = pos['metrics']['net_credit_or_debit']
                    if pos['metrics']['trade_type'] == "CREDIT":
...
class OptionsBacktest(QCAlgorithm):
    """Ratio Spread Backtesting Algorithm"""
...
{filter_init}
        ...
        self.Log("=" * 60)
        self.Log("Strategy: Ratio Spread (1:{{}})".format(config.deltas.get('ratio', 2)))
        ...
        self.Log("WARNING: This strategy has unlimited risk on the upside!")
        self.Log("=" * 60)
        self.Log("[+] Ready!")

        self.strategy = RatioSpreadStrategy(self)
