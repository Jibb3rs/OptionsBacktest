...
class IronCondor(BaseStrategy):
    """Iron Condor - Wide body, defined risk neutral strategy"""

    strategy_direction = 'neutral'  # Strategy expects neutral/range-bound conditions

    @staticmethod
    def generate_code(config):
        """Generate complete Iron Condor algorithm for QuantConnect"""
        ...
        # Map delta keys from GUI format to code format
        ...
        config_json = IronCondor._build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config)
        
        # Build a flattened config for base class methods
        flat_config = {
        ...
        indicator_manager = BaseStrategy.generate_indicator_manager(indicators_config, IronCondor.strategy_direction)
        advanced_filters = BaseStrategy.generate_advanced_filters(advanced_config)
        strike_selector = IronCondor._generate_strike_selector(advanced_config)
        strategy_class = IronCondor._generate_strategy_class()
        main_algorithm = IronCondor._generate_main_algorithm(flat_config, advanced_config)
...
"""
    ...
    def _build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config):
        ...
        # Build config string with proper escaping
        config_str = "{\n"
        config_str += f"        'ticker': '{config.get('ticker', 'SPY')}',\n"
        config_str += f"        'strategy': 'Iron Condor',\n"
        ...
        config_str += f"            'short_call': {abs(formatted_deltas.get('short_call', 0.16))},\n"
        config_str += f"            'long_call': {abs(formatted_deltas.get('long_call', 0.08))}\n"
        config_str += "        },\n"
    ...
    def _generate_strike_selector(advanced_config):
        """Generate strike selection logic for Iron Condor"""
        
        liquidity_enabled = advanced_config.get('liquidity_enabled', True)
        
        code = '''
class StrikeSelector:
    """Find strikes by delta for Iron Condor"""
    
    @staticmethod
    def find_strikes_by_delta(option_chain, underlying_price, target_deltas, config, algo):
        """Find IC strikes using delta targeting with same expiry enforcement"""
        ...
        try:
            ...
            # Find expiry with most contracts (best liquidity)
...
'''
        ...
        if liquidity_enabled:
            ...
            if len(liquid_puts) < 10 or len(liquid_calls) < 10:
...
'''
        ...
        code += '''
            # Find strikes from ONLY this expiry
            strikes = StrikeSelector._find_from_single_expiry(
                puts, calls, underlying_price, target_deltas, algo
            )
            
            if strikes:
                # Validate all have same expiry
                expiry_dates = {
                    strikes['short_put'].Expiry.date(),
                    strikes['long_put'].Expiry.date(),
                    strikes['short_call'].Expiry.date(),
                    strikes['long_call'].Expiry.date()
                }
                
                if len(expiry_dates) == 1:
                    algo.Log(f"[+] Valid Iron Condor! All legs expire {list(expiry_dates)[0]}")
                    return strikes
                else:
                    algo.Log(f"[!] WARNING: Mixed expiries found: {expiry_dates}")
                    return None
    ...
    @staticmethod
    def _find_from_single_expiry(puts, calls, underlying_price, target_deltas, algo):
        """Find all 4 strikes from a single expiration date"""
        
        # Find short put (OTM, ~16 delta)
        short_put = StrikeSelector._find_closest_delta(
            puts, target_deltas['short_put'], underlying_price, algo
        )
        
        # Find long put (further OTM, ~8 delta)
        long_put = StrikeSelector._find_closest_delta(
            puts, target_deltas['long_put'], underlying_price, algo
        )
        
        # Find short call (OTM, ~16 delta)
        short_call = StrikeSelector._find_closest_delta(
            calls, target_deltas['short_call'], underlying_price, algo
        )
        
        # Find long call (further OTM, ~8 delta)
        long_call = StrikeSelector._find_closest_delta(
            calls, target_deltas['long_call'], underlying_price, algo
        )
    ...
    @staticmethod
    def _find_closest_delta(contracts, target_delta, underlying_price, algo):
...
class IronCondorStrategy:
    """Manage Iron Condor positions"""
    
    def __init__(self, algorithm):
        self.algo = algorithm
    
    def enter_position(self, strikes, underlying_price, time):
        """Enter new Iron Condor position"""
        ...
        try:
            ...
            put_spread_width = (strikes['short_put'].Strike - strikes['long_put'].Strike) * 100
...
class OptionsBacktest(QCAlgorithm):
    """Iron Condor Backtesting Algorithm"""
...
{filter_init}
        ...
        self.Log("=" * 60)
        self.Log("Strategy: Iron Condor")
        self.Log(f"Ticker: {{config.ticker}}")
        self.Log(f"Strike Range: ±50 strikes")
        ...
        # Initialize strategy
        self.strategy = IronCondorStrategy(self)
