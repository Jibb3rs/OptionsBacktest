...
Buy OTM put + OTM call (volatility play, debit spread)
"""

from .base_strategy import BaseStrategy

class LongStrangle(BaseStrategy):
    """Long Strangle - Buy OTM puts and calls for volatility"""

    strategy_direction = 'neutral'  # Strategy expects neutral conditions before volatility spike

    @staticmethod
    def generate_code(config):
        """Generate complete Long Strangle algorithm"""
        ...
        if not formatted_deltas:
            formatted_deltas = {
                'long_put': 0.30,   # OTM put
                'long_call': 0.30   # OTM call
            }
        ...
        config_json = LongStrangle._build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config)
        ...
        indicator_manager = BaseStrategy.generate_indicator_manager(indicators_config, LongStrangle.strategy_direction)
        advanced_filters = BaseStrategy.generate_advanced_filters(advanced_config)
        strike_selector = LongStrangle._generate_strike_selector(advanced_config)
        strategy_class = LongStrangle._generate_strategy_class()
        main_algorithm = LongStrangle._generate_main_algorithm(flat_config, advanced_config)
...
"""
    ...
    def _build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config):
        ...
        config_str += f"        'strategy': 'Long Strangle',\n"
        ...
        config_str += "        'deltas': {\n"
        config_str += f"            'long_put': {abs(formatted_deltas.get('long_put', 0.30))},\n"
        config_str += f"            'long_call': {abs(formatted_deltas.get('long_call', 0.30))}\n"
        config_str += "        },\n"
...
'''
        ...
        code += '''
            ...
            if long_put and long_call:
                if long_put.Strike < underlying_price < long_call.Strike:
                    if long_put.Expiry.date() == long_call.Expiry.date():
                        algo.Log(f"[+] Valid Long Strangle! {long_put.Strike} (PUT) / {long_call.Strike} (CALL)")
                        return {
                            'long_put': long_put,
                            'long_call': long_call
                        }
...
class LongStrangleStrategy:
    ...
    def enter_position(self, strikes, underlying_price, time):
        ...
        try:
            ...
            net_debit = (long_put_price + long_call_price) * 100
            max_loss = net_debit
            max_profit = 999999  # Theoretically unlimited
            
            self.algo.Log("")
            self.algo.Log("=" * 60)
            self.algo.Log(">>> ENTERING LONG STRANGLE")
            self.algo.Log("=" * 60)
            ...
            self.algo.Log(f"Strikes: {strikes['long_put'].Strike} (PUT) / {strikes['long_call'].Strike} (CALL)")
        ...
        try:
            ...
            for chain in data.OptionChains:
                ...
                if len(current_prices) == 2:
                    current_value = (current_prices['lp'] + current_prices['lc']) * 100
...
class OptionsBacktest(QCAlgorithm):
    """Long Strangle Backtesting Algorithm"""
...
{filter_init}
        ...
        self.Log("=" * 60)
        self.Log("Strategy: Long Strangle")
        ...
        self.strategy = LongStrangleStrategy(self)
