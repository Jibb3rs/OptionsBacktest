"""
Custom Strategy Builder
Allows users to define custom option combinations with:
- Number of calls/puts
- Long or short positions
- Strike selection by delta
- Any combination of legs
"""

from .base_strategy import BaseStrategy

class CustomStrategy(BaseStrategy):
    """Custom Strategy - User-defined option combinations"""

    strategy_direction = 'neutral'  # default; overridden at generation time by _infer_direction

    @staticmethod
    def _infer_direction(legs, include_stock=False, stock_direction='long'):
        """
        Infer bullish/bearish/neutral from net delta of the custom legs.

        Delta contributions:
          long call  → +delta
          short call → -delta
          long put   → -delta
          short put  → +delta
          long stock → +0.5 equivalent
          short stock→ -0.5 equivalent
        """
        net_delta = 0.0
        for leg in legs:
            d = leg.get('delta', 0.5)
            t = leg.get('type', 'call')
            direction = leg.get('direction', 'long')
            if t == 'call':
                net_delta += d if direction == 'long' else -d
            else:  # put
                net_delta += -d if direction == 'long' else d

        if include_stock:
            net_delta += 0.5 if stock_direction == 'long' else -0.5

        if net_delta > 0.1:
            return 'bullish'
        elif net_delta < -0.1:
            return 'bearish'
        else:
            return 'neutral'

    @staticmethod
    def generate_code(config):
        """Generate complete Custom Strategy algorithm"""

        trading_rules = config.get('trading_rules', {})
        strike_selection = config.get('strike_selection', {})
        indicators_config = config.get('indicators', {})
        advanced_config = config.get('advanced', {})
        custom_config = config.get('custom_strategy', {})

        # Custom strategy legs configuration
        # Each leg: { 'type': 'call'/'put', 'direction': 'long'/'short', 'delta': 0.30, 'quantity': 1 }
        legs = custom_config.get('legs', [])
        include_stock = custom_config.get('include_stock', False)
        stock_quantity = custom_config.get('stock_quantity', 100)
        stock_direction = custom_config.get('stock_direction', 'long')  # 'long' or 'short'
        strategy_name = custom_config.get('name', 'Custom Strategy')

        if not legs and not include_stock:
            # Default to a simple long call if no legs defined
            legs = [{'type': 'call', 'direction': 'long', 'delta': 0.50, 'quantity': 1}]

        # Auto-infer strategy direction from leg composition
        inferred_direction = CustomStrategy._infer_direction(legs, include_stock, stock_direction)

        # Build deltas from legs
        formatted_deltas = {}
        for i, leg in enumerate(legs):
            key = f"leg_{i}_{leg['type']}_{leg['direction']}"
            formatted_deltas[key] = {
                'type': leg['type'],
                'direction': leg['direction'],
                'delta': leg.get('delta', 0.50),
                'quantity': leg.get('quantity', 1)
            }

        stop_loss_mode = trading_rules.get('stop_loss_mode', 'max_loss')
        stop_loss_map = {
            'Credit-Based (Options Standard)': 'credit',
            'Max Loss-Based (Conservative)': 'max_loss',
            'Equal Dollar (Balanced)': 'equal_dollar',
            'Price Distance (FX Style)': 'price_distance',
            'credit': 'credit',
            'max_loss': 'max_loss',
            'equal_dollar': 'equal_dollar',
            'price_distance': 'price_distance'
        }
        stop_loss_mode = stop_loss_map.get(stop_loss_mode, 'max_loss')

        config_json = CustomStrategy._build_config_json(
            config, formatted_deltas, stop_loss_mode, advanced_config,
            legs, include_stock, stock_quantity, stock_direction, strategy_name
        )

        flat_config = {
            'ticker': config.get('ticker', 'SPY'),
            'start_date': config.get('start_date'),
            'end_date': config.get('end_date'),
            'initial_capital': config.get('initial_capital', 100000),
            'expiry_days': config.get('expiry_days', 30),
            'expiry_range': config.get('expiry_range', 5),
            'resolution': trading_rules.get('resolution', 'Daily'),
            'max_positions': trading_rules.get('max_positions', 5),
            'min_days_between_trades': trading_rules.get('min_days_between_trades', 0),
            'profit_target_pct': trading_rules.get('profit_target_pct', 100),
            'stop_loss_mode': stop_loss_mode,
            'stop_loss_pct': trading_rules.get('stop_loss_pct', 50),
            'deltas': formatted_deltas
        }

        imports = BaseStrategy.generate_imports()
        config_loader = BaseStrategy.generate_config_loader().replace('{CONFIG_PLACEHOLDER}', config_json)
        indicator_manager = BaseStrategy.generate_indicator_manager(indicators_config, inferred_direction)
        advanced_filters = BaseStrategy.generate_advanced_filters(advanced_config)
        strike_selector = CustomStrategy._generate_strike_selector(advanced_config, legs)
        strategy_class = CustomStrategy._generate_strategy_class(legs, include_stock, stock_quantity, stock_direction, strategy_name)
        main_algorithm = CustomStrategy._generate_main_algorithm(flat_config, advanced_config, strategy_name, include_stock)

        algorithm_code = f"""{imports}

{config_loader}

{indicator_manager}

{advanced_filters}

{strike_selector}

{strategy_class}

{main_algorithm}
"""

        return algorithm_code

    @staticmethod
    def _build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config,
                           legs, include_stock, stock_quantity, stock_direction, strategy_name):
        """Build configuration dictionary as string"""

        trading_rules = config.get('trading_rules', {})

        config_str = "{\n"
        config_str += f"        'ticker': '{config.get('ticker', 'SPY')}',\n"
        config_str += f"        'strategy': '{strategy_name}',\n"
        config_str += f"        'expiry_days': {config.get('expiry_days', 30)},\n"
        config_str += f"        'expiry_range': {config.get('expiry_range', 5)},\n"
        config_str += f"        'resolution': '{trading_rules.get('resolution', 'Daily')}',\n"
        config_str += f"        'max_positions': {trading_rules.get('max_positions', 5)},\n"
        config_str += f"        'min_days_between_trades': {trading_rules.get('min_days_between_trades', 0)},\n"
        config_str += f"        'profit_target_pct': {trading_rules.get('profit_target_pct', 100)},\n"
        config_str += f"        'stop_loss_mode': '{stop_loss_mode}',\n"
        config_str += f"        'stop_loss_pct': {trading_rules.get('stop_loss_pct', 50)},\n"

        # Custom strategy specific config
        config_str += f"        'include_stock': {include_stock},\n"
        config_str += f"        'stock_quantity': {stock_quantity},\n"
        config_str += f"        'stock_direction': '{stock_direction}',\n"

        # Legs configuration
        config_str += "        'legs': [\n"
        for i, leg in enumerate(legs):
            config_str += "            {\n"
            config_str += f"                'type': '{leg['type']}',\n"
            config_str += f"                'direction': '{leg['direction']}',\n"
            config_str += f"                'delta': {leg.get('delta', 0.50)},\n"
            config_str += f"                'quantity': {leg.get('quantity', 1)}\n"
            config_str += "            }"
            if i < len(legs) - 1:
                config_str += ","
            config_str += "\n"
        config_str += "        ],\n"

        config_str += "        " + BaseStrategy.generate_advanced_config_string(advanced_config) + "\n"
        config_str += "    }"

        return config_str

    @staticmethod
    def _generate_strike_selector(advanced_config, legs):
        """Generate strike selection logic for custom legs"""

        liquidity_enabled = advanced_config.get('liquidity_enabled', True)

        code = '''
class StrikeSelector:
    """Find strikes for Custom Strategy"""

    @staticmethod
    def find_strikes_by_delta(option_chain, underlying_price, target_deltas, config, algo):
        """Find strikes for all custom legs"""

        try:
            expiries = {}
            for contract in option_chain:
                exp_date = contract.Expiry.date()
                if exp_date not in expiries:
                    expiries[exp_date] = {'puts': [], 'calls': []}

                if contract.Right == OptionRight.Put:
                    expiries[exp_date]['puts'].append(contract)
                else:
                    expiries[exp_date]['calls'].append(contract)

            # Apply expiration cycle filter
            if config.advanced['expiration_cycle'] != 'any':
                expiries = ExpirationFilter.filter_expiries(
                    expiries,
                    config.advanced['expiration_cycle']
                )

                if not expiries:
                    algo.Log("[X] No expiries match cycle filter")
                    return None

            best_expiry = max(expiries.keys(),
                            key=lambda x: len(expiries[x]['puts']) + len(expiries[x]['calls']))

            puts = expiries[best_expiry]['puts']
            calls = expiries[best_expiry]['calls']

            algo.Log(f"[+] Selected expiry: {best_expiry} ({len(puts)} puts, {len(calls)} calls)")
'''

        if liquidity_enabled:
            code += '''
            # Filter by liquidity
            liquid_puts = [p for p in puts if LiquidityFilter.check_liquidity(
                p,
                config.advanced['max_spread_pct'],
                config.advanced['min_open_interest']
            )]

            liquid_calls = [c for c in calls if LiquidityFilter.check_liquidity(
                c,
                config.advanced['max_spread_pct'],
                config.advanced['min_open_interest']
            )]

            puts = liquid_puts
            calls = liquid_calls
            algo.Log(f"[+] Liquid contracts: {len(puts)} puts, {len(calls)} calls")
'''

        advanced_greeks_enabled = advanced_config.get('advanced_greeks_enabled', False)
        if advanced_greeks_enabled:
            code += '''
            # Advanced Greeks Filter (contract-level)
            if hasattr(algo, 'advanced_greeks_filter'):
                puts = algo.advanced_greeks_filter.filter_contracts(puts)
                calls = algo.advanced_greeks_filter.filter_contracts(calls)
'''

        code += '''
            # Find contracts for each leg
            found_strikes = {}
            legs_config = config.legs

            for i, leg in enumerate(legs_config):
                leg_type = leg['type']
                leg_delta = leg['delta']
                leg_key = f"leg_{i}"

                if leg_type == 'call':
                    contract = StrikeSelector._find_closest_delta(calls, leg_delta, algo)
                else:  # put
                    contract = StrikeSelector._find_closest_delta(puts, leg_delta, algo)

                if contract:
                    found_strikes[leg_key] = contract
                    algo.Log(f"[+] Leg {i} ({leg['direction']} {leg_type}): Strike {contract.Strike}, Delta ~{leg_delta}")
                else:
                    algo.Log(f"[X] Could not find contract for leg {i}")
                    return None

            if len(found_strikes) == len(legs_config):
                algo.Log(f"[+] All {len(legs_config)} legs found!")
                return found_strikes

            return None

        except Exception as e:
            algo.Log(f"[X] Strike selection error: {e}")
            return None

    @staticmethod
    def _find_closest_delta(contracts, target_delta, algo):
        """Find contract with delta closest to target"""

        best_contract = None
        best_delta_diff = float('inf')

        for contract in contracts:
            if not contract.Greeks or not contract.Greeks.Delta:
                continue

            delta = abs(contract.Greeks.Delta)
            delta_diff = abs(delta - target_delta)

            if delta_diff < best_delta_diff:
                best_delta_diff = delta_diff
                best_contract = contract

        return best_contract
'''
        return code

    @staticmethod
    def _generate_strategy_class(legs, include_stock, stock_quantity, stock_direction, strategy_name):
        """Generate strategy class for custom configuration"""

        code = f'''
class CustomStrategyHandler:
    """Manage Custom Strategy positions"""

    def __init__(self, algorithm):
        self.algo = algorithm

    def enter_position(self, strikes, underlying_price, time):
        """Enter new Custom Strategy position"""

        try:
            if self.algo.IsWarmingUp:
                return None

            legs_config = self.algo.config.legs
            include_stock = self.algo.config.include_stock
            stock_qty = self.algo.config.stock_quantity
            stock_dir = self.algo.config.stock_direction

            total_debit = 0
            total_credit = 0
            leg_details = []

            self.algo.Log("")
            self.algo.Log("=" * 60)
            self.algo.Log(">>> ENTERING {strategy_name.upper()}")
            self.algo.Log("=" * 60)
            self.algo.Log(f"Underlying: ${{underlying_price:.2f}}")
'''

        # Add stock handling if included
        if include_stock:
            code += f'''
            # Stock position
            if include_stock:
                stock_cost = underlying_price * stock_qty
                if stock_dir == 'long':
                    self.algo.MarketOrder(self.algo.config.ticker, stock_qty)
                    total_debit += stock_cost
                    self.algo.Log(f"[+] Long {{stock_qty}} shares at ${{underlying_price:.2f}}")
                else:
                    self.algo.MarketOrder(self.algo.config.ticker, -stock_qty)
                    total_credit += stock_cost
                    self.algo.Log(f"[+] Short {{stock_qty}} shares at ${{underlying_price:.2f}}")
'''

        code += '''
            # Options legs
            for i, leg in enumerate(legs_config):
                leg_key = f"leg_{i}"
                contract = strikes[leg_key]
                price = (contract.BidPrice + contract.AskPrice) / 2
                qty = leg['quantity']
                direction = leg['direction']
                leg_type = leg['type'].upper()

                if direction == 'long':
                    self.algo.MarketOrder(contract.Symbol, qty)
                    total_debit += price * 100 * qty
                    self.algo.Log(f"[+] BUY {qty}x {leg_type} Strike {contract.Strike} @ ${price:.2f}")
                else:  # short
                    self.algo.MarketOrder(contract.Symbol, -qty)
                    total_credit += price * 100 * qty
                    self.algo.Log(f"[+] SELL {qty}x {leg_type} Strike {contract.Strike} @ ${price:.2f}")

                leg_details.append({
                    'contract': contract,
                    'direction': direction,
                    'quantity': qty,
                    'entry_price': price
                })

            # Calculate net cost
            net_cost = total_debit - total_credit

            if net_cost > 0:
                self.algo.Log(f"Net DEBIT: ${net_cost:.2f}")
            else:
                self.algo.Log(f"Net CREDIT: ${abs(net_cost):.2f}")

            self.algo.Log("[+] Orders placed")

            self.algo.position_counter += 1
            pos_id = self.algo.position_counter

            self.algo.positions[pos_id] = {
                'status': 'open',
                'entry_time': time,
                'entry_underlying_price': underlying_price,
                'expiry_date': strikes['leg_0'].Expiry.date(),
                'strikes': strikes,
                'leg_details': leg_details,
                'current_pnl': 0,
                'metrics': {
                    'net_cost': net_cost,
                    'total_debit': total_debit,
                    'total_credit': total_credit,
                    'max_profit': 999999,  # Depends on structure
                    'max_loss': max(net_cost, 0) if net_cost > 0 else 999999,
                    'mae': 0,
                    'mfe': 0
                }
            }

            self.algo.Log(f"Position #{pos_id} opened")
            self.algo.Log("=" * 60)

            return pos_id

        except Exception as e:
            self.algo.Log(f"[X] Entry error: {e}")
            return None

    def calculate_pnl(self, pos, data):
        """Calculate current P&L"""

        try:
            total_pnl = 0
            legs_config = self.algo.config.legs

            # Stock P&L if applicable
            if self.algo.config.include_stock:
                current_price = self.algo.Securities[self.algo.config.ticker].Price
                stock_pnl = (current_price - pos['entry_underlying_price']) * self.algo.config.stock_quantity
                if self.algo.config.stock_direction == 'short':
                    stock_pnl = -stock_pnl
                total_pnl += stock_pnl

            # Options P&L
            if data.OptionChains:
                for chain in data.OptionChains:
                    contracts = list(chain.Value)

                    for i, leg_detail in enumerate(pos['leg_details']):
                        entry_contract = pos['strikes'][f'leg_{i}']

                        for contract in contracts:
                            leg_type = legs_config[i]['type']
                            right = OptionRight.CALL if leg_type == 'call' else OptionRight.PUT

                            if (contract.Strike == entry_contract.Strike and
                                contract.Right == right and
                                contract.Expiry.date() == entry_contract.Expiry.date()):

                                current_price = (contract.BidPrice + contract.AskPrice) / 2
                                entry_price = leg_detail['entry_price']
                                qty = leg_detail['quantity']
                                direction = leg_detail['direction']

                                if direction == 'long':
                                    leg_pnl = (current_price - entry_price) * 100 * qty
                                else:
                                    leg_pnl = (entry_price - current_price) * 100 * qty

                                total_pnl += leg_pnl
                                break
                    break

            return total_pnl

        except Exception as e:
            self.algo.Log(f"[X] P&L calc error: {e}")
            return 0

    def close_position(self, pos_id, reason, time):
        """Close position"""

        try:
            pos = self.algo.positions[pos_id]
            legs_config = self.algo.config.legs

            # Close stock if applicable
            if self.algo.config.include_stock:
                qty = self.algo.config.stock_quantity
                if self.algo.config.stock_direction == 'long':
                    self.algo.MarketOrder(self.algo.config.ticker, -qty)
                else:
                    self.algo.MarketOrder(self.algo.config.ticker, qty)

            # Close option legs
            for i, leg_detail in enumerate(pos['leg_details']):
                contract = pos['strikes'][f'leg_{i}']
                qty = leg_detail['quantity']
                direction = leg_detail['direction']

                if direction == 'long':
                    self.algo.MarketOrder(contract.Symbol, -qty)
                else:
                    self.algo.MarketOrder(contract.Symbol, qty)

            final_pnl = pos['current_pnl']
            mae = pos['metrics']['mae']
            mfe = pos['metrics']['mfe']
            duration = (time.date() - pos['entry_time'].date()).days

            self.algo.Log("")
            self.algo.Log("=" * 60)
            self.algo.Log(f"<<< EXITING #{pos_id} - {reason}")
            self.algo.Log("=" * 60)
            self.algo.Log("[+] Exit complete")
            self.algo.Log(f"Duration: {duration} days")
            self.algo.Log(f"P&L: ${final_pnl:.2f}")
            self.algo.Log(f"MAE: ${mae:.2f}")
            self.algo.Log(f"MFE: ${mfe:.2f}")
            self.algo.Log("=" * 60)

            pos['status'] = 'closed'
            pos['exit_time'] = time
            pos['exit_reason'] = reason
            pos['final_pnl'] = final_pnl

        except Exception as e:
            self.algo.Log(f"[X] Exit error: {e}")
'''
        return code

    @staticmethod
    def _generate_main_algorithm(config, advanced_config, strategy_name, include_stock):
        """Generate main algorithm"""

        initialize_code = BaseStrategy.generate_initialize_base(config)
        position_tracking = BaseStrategy.generate_position_tracking()
        exit_logic = BaseStrategy.generate_exit_logic(config)

        filter_init = BaseStrategy.generate_filter_initialization(advanced_config)
        filter_checks = BaseStrategy.generate_filter_checks(advanced_config)

        code = f'''
class OptionsBacktest(QCAlgorithm):
    """{strategy_name} Backtesting Algorithm"""

    {initialize_code}



        # Initialize advanced filters
{filter_init}

        self.Log("=" * 60)
        self.Log("Strategy: {strategy_name}")
        self.Log(f"Ticker: {{config.ticker}}")
        self.Log(f"Expiry: {{config.expiry_days}} days (±{{config.expiry_range}} day window)")
        self.Log(f"Resolution: {{config.resolution.upper()}}")
        self.Log(f"Max Positions: {{config.max_positions}}")
        self.Log(f"Profit Target: {{config.profit_target_pct}}%")
        self.Log(f"Stop Loss: {{config.stop_loss_mode}} mode ({{config.stop_loss_pct}}%)")
        self.Log(f"Custom Legs: {{len(config.legs)}}")
        self.Log(f"Include Stock: {{config.include_stock}}")
        self.Log("=" * 60)
        self.Log("[+] Ready!")

        self.strategy = CustomStrategyHandler(self)

    def OnData(self, data):
        """Main data handler"""

        if self.IsWarmingUp:
            return

        if self.positions:
            self.update_positions(data)

        if not self.can_enter_new_trade():
            return

        if not data.OptionChains:
            return

        for chain in data.OptionChains:
            if chain.Key != self.option_symbol:
                continue

            underlying_price = self.Securities[self.config.ticker].Price

{filter_checks}

            strikes = StrikeSelector.find_strikes_by_delta(
                chain.Value,
                underlying_price,
                self.config.deltas if hasattr(self.config, 'deltas') else {{}},
                self.config,
                self
            )

            if not strikes:
                continue

            # Check indicators
            allowed, reason = self.indicators.should_enter_trade(self.config.ticker)
            if not allowed:
                self.Log(f"[X] Indicators blocked entry: {{reason}}")
                continue
            else:
                self.Log(f"[✓] Indicators: {{reason}}")

            self.strategy.enter_position(strikes, underlying_price, data.Time)
            self.last_trade_date = data.Time.date()

            break

    def can_enter_new_trade(self):
        """Check if we can enter a new trade"""

        open_positions = sum(1 for p in self.positions.values() if p['status'] == 'open')
        if open_positions >= self.config.max_positions:
            return False

        if self.last_trade_date and self.config.min_days_between_trades > 0:
            days_since_last = (self.Time.date() - self.last_trade_date).days
            if days_since_last < self.config.min_days_between_trades:
                return False

        return True

{position_tracking}

    def calculate_pnl(self, pos, data):
        """Calculate position P&L"""
        return self.strategy.calculate_pnl(pos, data)

{exit_logic}

    def close_position(self, pos_id, reason, time):
        """Close a position"""
        self.strategy.close_position(pos_id, reason, time)
'''

        return code
