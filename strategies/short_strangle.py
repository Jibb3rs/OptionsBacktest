...
Sell OTM put + OTM call (credit strategy)
"""

from .base_strategy import BaseStrategy

class ShortStrangle(BaseStrategy):
    """Short Strangle - Sell OTM puts and calls for credit"""

    strategy_direction = 'neutral'  # Strategy expects neutral/range-bound conditions

    @staticmethod
    def generate_code(config):
        """Generate complete Short Strangle algorithm"""
        ...
        config_json = ShortStrangle._build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config)
        ...
        indicator_manager = BaseStrategy.generate_indicator_manager(indicators_config, ShortStrangle.strategy_direction)
        advanced_filters = BaseStrategy.generate_advanced_filters(advanced_config)
        strike_selector = ShortStrangle._generate_strike_selector(advanced_config)
        strategy_class = ShortStrangle._generate_strategy_class()
        main_algorithm = ShortStrangle._generate_main_algorithm(flat_config, advanced_config)
...
"""
    ...
    def _build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config):
        ...
        config_str += f"        'strategy': 'Short Strangle',\n"
...
'''
        ...
        code += '''
            ...
            if short_put and short_call:
                if short_put.Strike < underlying_price < short_call.Strike:
                    if short_put.Expiry.date() == short_call.Expiry.date():
                        algo.Log(f"[+] Valid Short Strangle! {short_put.Strike} (PUT) / {short_call.Strike} (CALL)")
                        return {
                            'short_put': short_put,
...
class ShortStrangleStrategy:
    """Manage Short Strangle positions"""
    
    def __init__(self, algorithm):
        self.algo = algorithm
    
    def enter_position(self, strikes, underlying_price, time):
        """Enter new Short Strangle (CREDIT)"""
        ...
        try:
            ...
            self.algo.Log("")
            self.algo.Log("=" * 60)
            self.algo.Log(">>> ENTERING SHORT STRANGLE")
            self.algo.Log("=" * 60)
            ...
            self.algo.Log(f"Strikes: {strikes['short_put'].Strike} (PUT) / {strikes['short_call'].Strike} (CALL)")
...
class OptionsBacktest(QCAlgorithm):
    """Short Strangle Backtesting Algorithm"""
...
{filter_init}
        ...
        self.Log("=" * 60)
        self.Log("Strategy: Short Strangle")
        ...
        self.strategy = ShortStrangleStrategy(self)
