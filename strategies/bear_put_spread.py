...
class BearPutSpread(BaseStrategy):
    ...
    def generate_code(config):
        """Generate complete Bear Put Spread algorithm"""
        ...
        # CRITICAL FIX: Bear Put Spread needs HIGHER delta for long put
        if not formatted_deltas or formatted_deltas.get('long_put', 0) < 0.50:
            formatted_deltas = {
                'long_put': 0.70,    # ITM put to buy (high delta)
                'short_put': 0.30    # OTM put to sell (low delta)
            }
        
        # Validate: long_put should have HIGHER delta than short_put
        if formatted_deltas.get('long_put', 0) < formatted_deltas.get('short_put', 0):
            formatted_deltas['long_put'], formatted_deltas['short_put'] = \
                formatted_deltas['short_put'], formatted_deltas['long_put']
        
        # Ensure long_put is ITM/ATM (high delta)
        if formatted_deltas.get('long_put', 0) < 0.50:
            formatted_deltas['long_put'] = 0.70
        
        # Ensure short_put is OTM (low delta)
        if formatted_deltas.get('short_put', 0) > 0.50:
            formatted_deltas['short_put'] = 0.30
        ...
        config_json = BearPutSpread._build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config)
        ...
        indicator_manager = BaseStrategy.generate_indicator_manager(indicators_config, BearPutSpread.strategy_direction)
        advanced_filters = BaseStrategy.generate_advanced_filters(advanced_config)
        strike_selector = BearPutSpread._generate_strike_selector(advanced_config)
        strategy_class = BearPutSpread._generate_strategy_class()
        main_algorithm = BearPutSpread._generate_main_algorithm(flat_config, advanced_config)
...
"""
    ...
    def _build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config):
        ...
        config_str += f"        'strategy': 'Bear Put Spread',\n"
        ...
        config_str += "        'deltas': {\n"
        config_str += f"            'long_put': {abs(formatted_deltas.get('long_put', 0.70))},\n"
        ...
        code += '''
            ...
            if long_put and short_put:
                if short_put.Strike < long_put.Strike:
                    if long_put.Expiry.date() == short_put.Expiry.date():
                        algo.Log(f"[+] Valid Bear Put Spread! {short_put.Strike}/{long_put.Strike}")
                        return {
                            'long_put': long_put,
                            'short_put': short_put
                        }
...
class BearPutSpreadStrategy:
    """Manage Bear Put Spread positions"""
    
    def __init__(self, algorithm):
        self.algo = algorithm
    
    def enter_position(self, strikes, underlying_price, time):
        """Enter new Bear Put Spread (DEBIT)"""
        ...
        try:
            ...
            net_debit = (long_put_price - short_put_price) * 100
            spread_width = (strikes['long_put'].Strike - strikes['short_put'].Strike) * 100
            max_profit = spread_width - net_debit
            max_loss = net_debit
            
            self.algo.Log("")
            self.algo.Log("=" * 60)
            self.algo.Log(">>> ENTERING BEAR PUT SPREAD")
            self.algo.Log("=" * 60)
            self.algo.Log(f"Underlying: ${underlying_price:.2f}")
            self.algo.Log(f"Put Spread: {strikes['short_put'].Strike}/{strikes['long_put'].Strike}")
        ...
        try:
            ...
            for chain in data.OptionChains:
                ...
                if len(current_prices) == 2:
                    current_value = (current_prices['lp'] - current_prices['sp']) * 100
...
class OptionsBacktest(QCAlgorithm):
    """Bear Put Spread Backtesting Algorithm"""
...
{filter_init}
        ...
        self.Log("=" * 60)
        self.Log("Strategy: Bear Put Spread")
        ...
        self.strategy = BearPutSpreadStrategy(self)
