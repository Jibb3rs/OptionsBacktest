...
Buy ATM call + ATM put - profit from big move in either direction
"""

from .base_strategy import BaseStrategy

class LongStraddle(BaseStrategy):
    """Long Straddle - Buy ATM puts and calls for volatility"""

    strategy_direction = 'neutral'

    @staticmethod
    def generate_code(config):
        """Generate complete Long Straddle algorithm"""
        ...
        # Map delta keys - straddle uses ATM (0.50 delta)
        ...
        config_json = LongStraddle._build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config)
        ...
        indicator_manager = BaseStrategy.generate_indicator_manager(indicators_config, LongStraddle.strategy_direction)
        advanced_filters = BaseStrategy.generate_advanced_filters(advanced_config)
        strike_selector = LongStraddle._generate_strike_selector(advanced_config)
        strategy_class = LongStraddle._generate_strategy_class()
        main_algorithm = LongStraddle._generate_main_algorithm(flat_config, advanced_config)
...
"""
    ...
    def _build_config_json(config, formatted_deltas, stop_loss_mode, advanced_config):
        ...
        config_str += f"        'strategy': 'Long Straddle',\n"
...
'''
        ...
        code += '''
            ...
            atm_strike = min([c.Strike for c in calls], key=lambda x: abs(x - underlying_price))

            # Find put and call at ATM strike
            long_put = None
            long_call = None

            for p in puts:
                if p.Strike == atm_strike:
                    long_put = p
                    break
            ...
            if long_put and long_call:
                algo.Log(f"[+] Valid Long Straddle! ATM Strike: {atm_strike}")
                return {
                    'long_put': long_put,
                    'long_call': long_call
                }
...
class LongStraddleStrategy:
    """Manage Long Straddle positions"""

    def __init__(self, algorithm):
        self.algo = algorithm

    def enter_position(self, strikes, underlying_price, time):
        """Enter new Long Straddle (DEBIT)"""
        ...
        try:
            ...
            max_loss = net_debit
            max_profit = 999999  # Theoretically unlimited on upside

            self.algo.Log("")
            self.algo.Log("=" * 60)
            self.algo.Log(">>> ENTERING LONG STRADDLE")
            self.algo.Log("=" * 60)
            self.algo.Log(f"Underlying: ${underlying_price:.2f}")
            self.algo.Log(f"ATM Strike: {strikes['long_put'].Strike}")
...
class OptionsBacktest(QCAlgorithm):
    """Long Straddle Backtesting Algorithm"""
...
{filter_init}
        ...
        self.Log("=" * 60)
        self.Log("Strategy: Long Straddle")
        ...
        self.strategy = LongStraddleStrategy(self)
