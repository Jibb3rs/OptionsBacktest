...
class CompareTab(QWidget):
    ...
    def _create_ui(self):
        ...
        # Drop zone: stacked widget (empty-state label / file list)
        self._list_stack = QStackedWidget()
        self._list_stack.setMinimumHeight(110)
        self._list_stack.setMaximumHeight(130)

        # Layer 0 — empty state
        empty_lbl = QLabel("Drop CSVs here or click  Add CSV")
        empty_lbl.setAlignment(Qt.AlignCenter)
        empty_lbl.setStyleSheet(
            f"color: {C['dim']}; font-size: 13px; border: none; background: transparent;"
        )
        self._list_stack.addWidget(empty_lbl)
        ...
        file_row.addWidget(self._list_stack, 1)
    ...
    def _add_csv(self):
        ...
        self._sync_list_layer()

    def _remove_selected(self):
        for item in self.file_list.selectedItems():
            filename = item.text()
            self.loaded_results.pop(filename, None)
            self.file_list.takeItem(self.file_list.row(item))
        self._sync_list_layer()

    def _sync_list_layer(self):
        """Switch between empty-state label and file list."""
        self._list_stack.setCurrentIndex(1 if self.file_list.count() > 0 else 0)
    ...
    def _update_equity_chart(self):
        ...
        for name, df in self.loaded_results.items():
            ...
            if pnl_col:
                series_data.append({'name': name, 'data': df[pnl_col].cumsum().tolist()})

        if series_data:
            max_len = max(len(s['data']) for s in series_data)
            option = create_line_chart_option(
                "Equity Curves Comparison",
                [str(i) for i in range(1, max_len + 1)],
                series_data,
                "Cumulative P&L ($)"
            )
    ...
    def _update_metrics_chart(self):
        metrics = {'Total P&L': [], 'Win Rate': [], 'Profit Factor': []}
        names = []
        ...
        for name, df in self.loaded_results.items():
            ...
            if pnl_col:
                ...
                metrics['Win Rate'].append(
                    round(len(wins) / len(df) * 100 if len(df) > 0 else 0, 1)
                )
    ...
    def _update_comparison_table(self):
        ...
        self.compare_table.setHorizontalHeaderLabels(
            ['Metric'] + [n.replace('.csv', '') for n in names]
        )
        ...
        metrics = [
            ...
            'Total P&L', 'Average P&L', 'Avg Winner', 'Avg Loser',
            'Max Win', 'Max Loss', 'Profit Factor', 'Sharpe Ratio',
        ]
